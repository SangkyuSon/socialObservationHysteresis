...
if options.compare & isempty(options.data2),
    options.data2 = data(:,2);
    data = data(:,1);
    barNo = 1;
    options.x = 1;
end
...
if options.showbt
    [~,~,data] = sem(data,'no',size(data,1),'std',options.std);
end
...
for bn = 1:barNo,
    ...
    if ~isnan(options.scatter), 
        ...
        if options.scatter ~= 1,
            ...
            xs = repmat(options.x(bn),length(data(:,bn)),1)+1*options.scatter/2*(randn(length(data(:,bn)),1)*2-1);
...
else, ylim([nanmin(nanmin([mu-ci]))*(1-sign(nanmin(nanmin([mu-ci])))*0.1),nanmax(nanmax([mu+ci]))*(1+sign(nanmax(nanmax([mu+ci])))*0.3)]); end
...
if options.errorbar, 
    xlimval = mean(diff(options.x))/2;
    try,
    xlim([options.x(1)-xlimval,options.x(end)+xlimval]);
    end
end
end